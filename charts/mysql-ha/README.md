...
=================
   ...
   * [在 Kubernetes 上部署 DBKernel MySQL 集群](#在-kubernetes-上部署-dbkernel-mysql-集群)
      * [简介](#简介)
      * [部署准备](#部署准备)
      * [部署步骤](#部署步骤)
         * [通过 Git 部署](#通过-git-部署)
            * [步骤 1：克隆 DBKernel MySQL Chart](#步骤-1克隆-dbkernel-mysql-chart)
            * [步骤 2：部署](#步骤-2部署)
         * [通过 repo 部署](#通过-repo-部署)
            * [步骤 1 : 添加仓库](#步骤-1--添加仓库)
            * [步骤 2 : 部署](#步骤-2--部署)
         * [部署校验](#部署校验)
      * [连接 DBKernel MySQL](#连接-dbkernel-mysql)
         * [客户端与 DBKernel MySQL 在同一 NameSpace 中](#客户端与-dbkernel-mysql-在同一-namespace-中)
         * [客户端与 DBKernel MySQL 不在同一 NameSpace 中](#客户端与-dbkernel-mysql-不在同一-namespace-中)
      * [配置](#配置)
      * [持久化](#持久化)
      * [自定义 MYSQL 配置](#自定义-mysql-配置)

# 在 Kubernetes 上部署 DBKernel MySQL 集群

## 简介

DBKernel MySQL 是基于 MySQL 的开源、高可用、云原生集群解决方案。通过使用 Raft 高可用选举协议的 DBKernel MySQL 可以快速进行故障转移，且不会丢失任何事务。

本教程演示如何使用命令行在 Kubernetes 上部署 DBKernel MySQL。
...
#### 步骤 1：克隆 DBKernel MySQL Chart

执行如下命令，将 DBKernel MySQL Chart 克隆到 Kubernetes 中。

```bash
git clone https://github.com/dbkernel/helm-charts
```
...
在 helm-charts 目录路径下，选择如下方式，部署 release 实例。

> release 是运行在 Kubernetes 集群中的 Chart 的实例。通过命令方式部署，需指定 release 名称。

以下命令指定 release 名为 `demo`，将创建一个名为 `demo-mysql-ha` 的有状态副本集。
...
```bash
$ helm install demo dbkernel/mysql-ha
NAME: demo
LAST DEPLOYED: Sun Aug 29 00:19:22 2021
NAMESPACE: default
STATUS: deployed
REVISION: 1
TEST SUITE: None
...
The cluster is comprised of 3 pods: 1 leader and 2 followers. Each instance is accessible within the cluster through:
    ...
    <pod-name>.demo-mysql-ha

To connect to your database:

1. Get mysql user `admin`'s password:

    kubectl get secret -n default demo-mysql-ha -o jsonpath="{.data.mysql-password}" | base64 --decode; echo
...
4. To connect to leader service in the Ubuntu pod:
    ...
    mysql -h demo-mysql-ha-leader -u admin -p

5. To connect to follower service (read-only) in the Ubuntu pod:

    mysql -h demo-mysql-ha-follower -u admin -p
```

分别执行如下指令，查看到 `release` 名为 `demo` 的有状态副本集 `demo-mysql-ha`，则 DBKernel MySQL 部署成功。

```bash
$ helm list
NAME               	NAMESPACE	REVISION	UPDATED                                	STATUS  	CHART                     	APP VERSION
demo               	default  	1       	2021-08-29 00:19:22.660724784 +0800 CST	deployed	mysql-ha-1.0.0            	5.7.34

$ kubectl get statefulset
NAME                 READY   AGE
demo-mysql-ha   3/3     15m
```

### 部署校验

部署指令执行完成后，查看 DBKernel MySQL 有状态副本集，pod 状态及服务。可查看到相关信息，则 DBKernel MySQL 部署成功。
...
您需要准备一个用于连接 DBKernel MySQL 的客户端。

### 客户端与 DBKernel MySQL 在同一 NameSpace 中

当客户端与 DBKernel MySQL 集群在同一个 NameSpace 中时，可使用 leader/follower service 名称代替具体的 ip 和端口。
...
### 客户端与 DBKernel MySQL 不在同一 NameSpace 中

当客户端与 DBKernel MySQL 集群不在同一个 NameSpace 中时，需先分别获取连接所需的节点地址、节点端口、服务名称。
...
下表列出了 DBKernel MySQL Chart 的配置参数及对应的默认值。
...
| `mysql.image`                                | `mysql` 镜像库地址                                         | `dbkernel/percona`                     |
| `mysql.tag`                                  | `mysql` 镜像标签                                          | `5.7.34`                               |
| `mysql.allowEmptyRootPassword`               | 如果为 `true`，允许 root 账号密码为空                       | `true`                                  |
| `mysql.mysqlRootPassword`                    | `root` 用户密码                                          |                                          |
| `mysql.mysqlReplicationPassword`             | `qc_repl` 用户密码                                         | `Rep1@2o2l`, 如果没有设置则随机12个字符      |
| `mysql.mysqlUser`                            | 新建用户的用户名                                           | `admin`                              |
| `mysql.mysqlPassword`                        | 新建用户的密码                                             | `User@2o2l`, 如果没有设置则随机12个字符      |
| `mysql.mysqlDatabase`                        | 将要创建的数据库名                                          | `dbkernel`                             |
...
| `xenon.image`                                | `xenon` 镜像库地址                                          | `dbkernel/xenon`                       |
| `xenon.tag`                                  | `xenon` 镜像标签                                            | `1.1.5`                          |
...
[MySQL](https://hub.docker.com/r/dbkernel/percona) 镜像在容器路径 `/var/lib/mysql` 中存储 MYSQL 数据和配置。
